...
namespace IntervalChangePusherTestCore
{
    [TestClass]
    public class IntervalChangePusherTest
    {
        ...
        public void TestLoadData()
        {
            ...
            intervalChangePusher.Subscribe(listener, TradeTopic);
            intervalChangePusher.Subscribe(listener, DetailTopic);
            Assert.AreEqual(initialBidAsk, intervalChangePusher.Fetch(BidAskTopic, "1"));
            Assert.AreEqual(initialDetail, intervalChangePusher.Fetch(DetailTopic, "3"));
            Assert.AreEqual(initialTrade, intervalChangePusher.Fetch(TradeTopic, "2"));

            intervalChangePusher.Put(BidAskTopic, "1", "test10");
            Assert.AreEqual("test10", intervalChangePusher.Fetch(BidAskTopic, "1"));
            intervalChangePusher.Put(TradeTopic, "2", "test20");
            Assert.AreEqual("test20", intervalChangePusher.Fetch(TradeTopic, "2"));
            intervalChangePusher.Put(DetailTopic, "3", "test30");
            Assert.AreEqual("test30", intervalChangePusher.Fetch(DetailTopic, "3"));
            lstResult.Add($"start time:{DateTime.Now.TimeOfDay}");
            Task.Delay(1200).Wait();
            intervalChangePusher.Put(BidAskTopic, "1", "test11");
            intervalChangePusher.Put(TradeTopic, "2", "test21");
            Assert.AreEqual("test21", intervalChangePusher.Fetch(TradeTopic, "2"));
            Task.Delay(500).Wait();
            intervalChangePusher.Put(BidAskTopic, "1", "test12");
            Assert.AreEqual("test12", intervalChangePusher.Fetch(BidAskTopic, "1"));
            intervalChangePusher.Put(TradeTopic, "2", "test22");
            intervalChangePusher.Put(DetailTopic, "3", "test31");
            Task.Delay(2000).Wait();
            intervalChangePusher.Put(DetailTopic, "3", "test32");
            Assert.AreEqual("test32", intervalChangePusher.Fetch(DetailTopic, "3"));
        ...
        public void LoadTest()
        {
            ...
            while (lstValue.Count > 0)
            {
                ...
                counter++;
                if (counter % 79 == 0)
                    lstLoads.Add(intervalChangePusher.Fetch(BidAskTopic,
