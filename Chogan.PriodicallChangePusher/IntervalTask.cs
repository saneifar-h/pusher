...
namespace PeriodicalChangePusher.Core
{
    public class IntervalTask
    {
        ...
        private readonly ConcurrentDictionary<string, KeyValuePair<long, string>> keyValues;
        ...
        public IntervalTask(string topic, int interval,
            ConcurrentDictionary<string, KeyValuePair<long, string>> keyValues)
        {
        ...
        private void TaskAction()
        {
            while (true)
            {
                ...
                var lstValues = new List<KeyValuePair<string, string>>();
                foreach (var item in keyValues.ToArray())
                {
                    if (processedValues.ContainsKey(item.Key) && processedValues[item.Key] >= item.Value.Key)
                        continue;
                    lstValues.Add(new KeyValuePair<string, string>(item.Key, item.Value.Value));
